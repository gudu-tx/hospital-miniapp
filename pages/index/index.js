...
Page({
  ...
  getGoods: function() {
    var that = this;
    wx.request({
      url: app.globalData.domain + '/api/hospital/list',
      data: {
      ...
      success: function(res) {
        ...
        var goods = that.data.hospitalList;
        for (var i = 0; i < res.data.hospitalList.length; i++) {
          goods.push(res.data.hospitalList[i]);
        }
        that.setData({
          hospitalList: goods,
          isLoad: false
